...
{
	GtkWidget *window, *packer, *packer2, *button;
	GdkColor color;
	GtkStyle *style;
	...
	gtk_widget_set_style( window, style );
 	...
 	gtk_widget_set_usize(packer, 150, 50);

 	options = GTK_FILL_X;

 	packer2 = gtk_packer_new();
	gtk_packer_add( GTK_PACKER( packer ), packer2,
		GTK_SIDE_TOP, GTK_ANCHOR_CENTER, options, 0, 0, 0, 0, 5 );
	...
	gtk_packer_add( GTK_PACKER( packer2 ), button,
		GTK_SIDE_LEFT, GTK_ANCHOR_CENTER, options, 0, 0, 0, 0, 10 );

	button = gtk_button_new_with_label( "beta" );
	gtk_widget_show( button );
	gtk_packer_add( GTK_PACKER( packer2 ), button,
		GTK_SIDE_RIGHT, GTK_ANCHOR_CENTER, options, 0, 0, 0, 10, 0 );
