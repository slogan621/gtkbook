...
main( int argc, char *argv[] )
{
  ...
  args = gtk_object_query_args( gtk_button_get_type(), &flags, &nArgs );
	...
	for ( i = 0; i < nArgs; i++ ) {
		...
		if ( flags[i] == GTK_ARG_READABLE || flags[i] == GTK_ARG_READWRITE ) {
			...
			case GTK_TYPE_STRING :
				{ 
				  ...
				  GTK_VALUE_STRING(args[i]) = "Yabba Dabba Doo";
				  gtk_object_setv( GTK_OBJECT( widget ), 1, &args[i] );
				  gtk_object_arg_get( GTK_OBJECT( widget ), &args[i], NULL );
				  printf( "%s\n", GTK_VALUE_STRING (args[i]) );
				  g_free(GTK_VALUE_STRING (args[i]));
				  ...
				  GTK_VALUE_STRING(arg) = "Hey Wilma!";
				  gtk_object_setv( GTK_OBJECT( widget ), 1, &arg );
				  gtk_object_arg_get( GTK_OBJECT( widget ), &arg, NULL );
				  printf( "%s\n", GTK_VALUE_STRING (arg) );
				  g_free (GTK_VALUE_STRING (arg));
				  gtk_object_set( GTK_OBJECT( widget ),
					  "GtkButton::label", "My friend Barney", NULL );
				  gtk_object_arg_get( GTK_OBJECT( widget ), &arg, NULL );
				  printf( "%s\n", GTK_VALUE_STRING (arg) );
				  g_free (GTK_VALUE_STRING (arg));
				}
